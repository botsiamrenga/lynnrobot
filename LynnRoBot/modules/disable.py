...
from LynnRoBot.modules.helper_funcs.handlers import CMD_STARTERS, SpamChecker
from LynnRoBot.modules.helper_funcs.misc import is_module_loaded
...
if is_module_loaded(FILENAME):
    ...
    from LynnRoBot.modules.sql import disable_sql as sql
    ...
    def disable_module(update: Update, context: CallbackContext):
        ...
        chat = update.effective_chat
        if len(args) >= 1:
            disable_module = "LynnRoBot.modules." + args[0].rsplit(".", 1)[0]
    ...
    def enable_module(update: Update, context: CallbackContext):
        ...
        if len(args) >= 1:
            enable_module = "LynnRoBot.modules." + args[0].rsplit(".", 1)[0]
    ...
    __help__ = """
    ☞︎︎︎ /cmds*:* check the current status of disabled commands
    *Admins only:*
    ☞︎︎︎ /enable <cmd name>*:* enable that command
    ☞︎︎︎ /disable <cmd name>*:* disable that command
    ☞︎︎︎ /enablemodule <module name>*:* enable all commands in that module
    ☞︎︎︎ /disablemodule <module name>*:* disable all commands in that module
    ☞︎︎︎ /listcmds*:* list all possible toggleable commands
    """
