from LynnRoBot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from LynnRoBot import DRAGONS
...
async def Lynn_is_admin(chat_id: int):
    status = False
    Lynn = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Lynn.id == user.id:
